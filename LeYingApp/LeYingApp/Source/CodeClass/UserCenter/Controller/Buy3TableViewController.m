...
#import "TestModel.h"
#import "orderModer.h"
@interface Buy3TableViewController ()
...
@implementation Buy3TableViewController
...
- (void)viewDidLoad {
    ...
    else
    {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [self getArtcollectWithState:@"2"Page:@"1"];
...
- (void)footerRereshing
{
    ...
    else
    {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [self getArtcollectWithState:@"2"Page:[NSString stringWithFormat:@"%ld",(long)self.index++]];
            dispatch_async(dispatch_get_main_queue(), ^{
                // 刷新表格
                [self.tableView reloadData];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return 280 + [self heightForString:self.contentStr];
}
