...
#import "AlertShow.h"
@interface Order2ViewController ()<UITextViewDelegate,SDCycleScrollViewDelegate,UIScrollViewDelegate>

@property (nonatomic,strong)order2View *ov;
...
- (void)viewDidLoad
{
    ...
    self.ov = [[order2View alloc]initWithFrame:[UIScreen mainScreen].bounds];
    self.view = _ov;
...
-(void)addBtnAction:(UIButton *)sender
{
    ...
    else
    {
        ...
        order.showMassageLab.text = [NSString stringWithFormat:@"%@%ld%@",@"共",[order.mountLab.text integerValue],@"件商品"];
        order.allLab.text = [NSString stringWithFormat:@"%f",[order.mountLab.text integerValue] * [order.priceLab.text floatValue]];
    }
...
-(void)affirmBtnAction:(UIButton *)sender
{
    ...
    {
        [AlertShow alertShowWithContent:@"身份证号不能为空" Seconds:2];
    ...
    else
    {
        ...
        {
            if (self.ov.consigneeText.text.length != 18) {
                [AlertShow alertShowWithContent:@"请输入正确的身份证号" Seconds:2];
            }
            else
            {
                if (![[IsHaveNetwork shareIsHaveNetwork] isConnectionAvailable])
